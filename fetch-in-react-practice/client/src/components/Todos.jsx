...
export default function Todos() {
  ...
  async function addTodo(newTodo) {
    try {
      ...
      const todosTotal = todos.concat(result);
      setTodos(todosTotal);
    } catch (error) {
      setError(error);
    }
  ...
  async function toggleCompleted(todoId) {
    try {
      const completeStatus = todos.find((e) => e.todoId === todoId).isCompleted;
      if (!completeStatus) {
        throw new Error('completeStatus not found');
      }
      ...
      const result = await response.json();
      const todosTotal = todos.map((e) => (e.todoId === todoId ? result : e));
      setTodos(todosTotal);
    } catch (error) {
      setError(error);
    }
